...
public class StandardRouteService {
    ...
    public StandardRouteService(StandardRouteRepository standardRouteRepository, RouteStopRepository routeStopRepository, UserAccountRepository userAccountRepository, AdministratorRepository administratorRepository, StudentRepository studentRepository, StandardRouteRequestMapper standardRouteRequestMapper, StandardRouteResponseMapper standardRouteResponseMapper, CurrentUserService currentUserService, MapboxAPIService mapboxAPIService) {
    ...
    public Page<StandardRouteResponseDTO> getAllStandardRoutes(Pageable pageable) {
        String authenticatedUserEmail = getAuthenticatedUserEmail();

        getAndValidateActiveStudent(authenticatedUserEmail);
    ...
    public StandardRouteResponseDTO getStandardRouteById(UUID standardRouteId) {
        ...
        validateSameCustomer(student.getCustomerId(), standardRoute.getCustomerId());

        return standardRouteResponseMapper.toDTO(standardRoute);
    }

    @Transactional(readOnly = true)
    public StandardRouteResponseDTO getStandardRouteStopPoints(UUID standardRouteId, GeneralStatus status, TravelDirection travelDirection) {
        String authenticatedUserEmail = getAuthenticatedUserEmail();

        Student student = getAndValidateActiveStudent(authenticatedUserEmail);

        StandardRoute baseRoute = standardRouteRepository.findRouteBaseByIdAndStatusAndTravelDirection(standardRouteId, status, travelDirection)
                .orElseThrow(() -> new EntityNotFoundException("Rota padrão não encontrada para a direção: " + travelDirection));

        validateSameCustomer(student.getCustomerId(), baseRoute.getCustomerId());

        Set<RouteStopAssignmentResponseDTO> stops = standardRouteRepository.findAssignmentsByRouteIdAndTravelDirection(standardRouteId, travelDirection);
        ...
        return new StandardRouteResponseDTO(
                ...
                baseRoute.getStandardGeometryOutbound(),
                baseRoute.getStandardGeometryReturn(),
    ...
    @Transactional
    public StandardRouteResponseDTO createStandardRoute(StandardRouteRequestDTO standardRouteRequestDTO) {
        String authenticatedEmail = getAuthenticatedUserEmail();

        // verifica se é um admin válido
        checkValidAdmin(authenticatedEmail);
        ...
        TravelDirection travelDirection = standardRouteRequestDTO.travelDirection();

        // garante que todos os pontos de parada informados pertencem à mesma direção da rota
        boolean allSameTravelDirection = standardRouteRequestDTO.routeStops().stream()
                .allMatch(routeStop -> routeStop.travelDirection() == travelDirection);

        if (!allSameTravelDirection) {
            throw new InvalidTravelDirectionException("Todos os pontos de parada devem pertencer à mesma direção da Rota Padrão: " + travelDirection);
        }
        ...
        /*
         * valida a ordem de parada não deixando ela se repetir
         * (ex.: parada 1 (0), parada 4(0) e não deixando ser null
         * */
        ...
        List<RouteStopAssignment> assignments = standardRouteRequestDTO.routeStops().stream()
                .map(request -> {
                    ...
                    assignment.setTravelDirection(request.travelDirection());
        ...
        /*
         * com base na direção informada, faz a persistência do geometry correto
         * outbound = origin primeiro, return = destination primeiro
         * */
        if (travelDirection == TravelDirection.OUTBOUND) {

            RouteDetailsDTO routeDetailsDTO = calculateStandardRouteGeometry(
                    standardRouteRequestDTO.originLongitude(),
    ...
    @Transactional
    public StandardRouteResponseDTO updateStandardRoute(UUID standardRouteId, TravelDirection travelDirection, StandardRouteUpdateDTO standardRouteUpdateDTO) {
        String authenticatedUserEmail = getAuthenticatedUserEmail();

        // verifica se é um válido
        checkValidAdmin(authenticatedUserEmail);

        if (travelDirection == null) {
            throw new TravelDirectionRequiredException("É necessário inserir a direção para essa operação");
        }
        ...
        // só recalcula a geometria (chamada externa ao Mapbox) se origem ou destino de fato mudaram nesta requisição
        boolean coordinatesChanged = originLatitudeFromDTO || destinationLatitudeFromDTO;

        /*
        * as coordenadas só são atualziadas quando são efetivmente modificadas
        * o mapstruct está configurado para ignora-las devido a operação complexa
        * */
        if (coordinatesChanged) {
            // busca os assignments com base na direção informada, em caso de null lança List.of para evitar NPE
            List<RouteStopAssignment> routeStopAssignments = standardRoute.getRouteStopAssignments() != null
                    ...
                    .filter(rs -> rs.getTravelDirection() == travelDirection)
                    .sorted(Comparator.comparing(RouteStopAssignment::getSequence))
                    .toList()
                    : List.of();
