...
public interface StudentRouteStopAssignmentRepository extends JpaRepository<StudentRouteStopAssignment, UUID> {
    ...
    long countByStudentIdAndTravelDirection(@Param("studentId") UUID studentId, @Param("travelDirection") TravelDirection travelDirection);

    List<StudentRouteStopAssignment> findByStudentIdAndStandardRouteIdAndTravelDirection(
            UUID studentId,
...
""")
    ...
    WHERE a.student.id = :studentId 
      ...
      AND a.travelDirection = :travelDirection
      AND a.id != :currentAssignmentId
""")
    boolean existsByStudentIdAndStandardRouteIdAndTravelPeriodAndTravelDirectionAndIdNot(
            @Param("studentId") UUID studentId,
            @Param("standardRouteId") UUID standardRouteId,
            @Param("travelPeriod") TravelPeriod travelPeriod,
            @Param("travelDirection") TravelDirection travelDirection,
